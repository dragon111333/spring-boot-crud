...
import com.testsdemo.testcrud.repo.GuildInfoRepo;
...
public class GuildInfoService {
	@Autowired
	private GuildInfoRepo guildInfoRepo;
	@Autowired
	...
	public Iterable<GuildInfo> getAllByUserid(int userId) {
		String sql ="SELECT e.*FROM guild_info e WHERE e.user_id = :userId";
	...
	public GuildInfo add(CreateGuildInfoDto e) {		
		try {
			User user = this.usersService.getById(e.getUserId());
			if(user == null) throw new Error("not found user");
			GuildInfo n = new GuildInfo(e.getName(),user);
			this.guildInfoRepo.save(n);
			return n;
		}catch(Exception ex) {
			ex.printStackTrace();
			return null;
	...
	public int delete(int userId,int id) {
		String sql ="delete from guild_info where id = :id and user_id = :userId";
		System.out.println("SQL : %s".formatted(sql));
